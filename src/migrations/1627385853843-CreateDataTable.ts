...
export class CreateDataTable1627385853843 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      "CREATE TABLE `site_info_entity` (`createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `id` int UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Primary key', `userId` int NOT NULL COMMENT 'UCenter user id', `title` varchar(255) NOT NULL COMMENT 'Site title', `subtitle` varchar(255) NOT NULL COMMENT 'Site subtitle' DEFAULT '', `description` text NOT NULL COMMENT 'Site description', `author` varchar(255) NOT NULL COMMENT 'Site author' DEFAULT '', `keywords` text NULL COMMENT 'Site keywords', `favicon` varchar(255) NULL COMMENT 'Site favicon link', PRIMARY KEY (`id`)) ENGINE=InnoDB",
    );
    await queryRunner.query(
