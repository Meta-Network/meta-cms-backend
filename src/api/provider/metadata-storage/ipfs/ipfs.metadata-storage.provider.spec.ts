...
describe('IpfsMetadataStorageProvider', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [IpfsMetadataStorageProvider],
    }).compile();

    configService = module.get<ConfigService>(ConfigService);
    ipfsMetadataStorageProvider = module.get<IpfsMetadataStorageProvider>(
  ...
  describe('get', () => {
    ...
    it('Could get QmeoZEDYMxAdftJFSL874zFXhnQkK1C1Sj9wix8TDgQUpW', async () => {
      expect(
        await ipfsMetadataStorageProvider.get(
  ...
  describe('upload', () => {
    ...
    it('Could upload plain text', async () => {
      ...
      const cid = await ipfsMetadataStorageProvider.upload(
        'alice-posts-1-sign',
        content,
      );
      expect(await ipfsMetadataStorageProvider.get(cid)).toEqual(content);
    });
  });
});
